 ...
 */
function canis_handle_theme_update()
{
...
add_action( 'admin_init', 'canis_handle_theme_update' );

/***********************************************
* Add our menu item
***********************************************/

function canis_license_menu() {
	add_theme_page( 'Theme License', 'Theme License', 'manage_options', 'themename-license', 'canis_license_page' );
}
add_action('admin_menu', 'canis_license_menu');

/***********************************************
* Sample settings page, substitute with yours
***********************************************/

function canis_license_page() {
	$license 	= get_option( BLANK_THEME_PRO_PREFIX . '_license_key' );
	$status 	= get_option( BLANK_THEME_PRO_PREFIX . '_license_key_status' );
	?>
	<div class="wrap">
		<h2><?php _e('Theme License Options', 'canis'); ?></h2>
				...
				<tbody>
					<tr valign="top">
						...
						<td>
							...
							<label class="description" for="<?php echo BLANK_THEME_PRO_PREFIX; ?> _license_key"><?php _e('Enter your license key', 'canis'); ?></label>
						</td>
					</tr>
					<?php if( false !== $license ) { ?>
						<tr valign="top">
							<th scope="row" valign="top">
								<?php _e('Activate License', 'canis'); ?>
							</th>
							<td>
								<?php if( $status !== false && $status == 'valid' ) { ?>
									<span style="color:green;"><?php _e('active', 'canis'); ?></span>
									<?php wp_nonce_field( 'edd_sample_nonce', 'edd_sample_nonce' ); ?>
									<input type="submit" class="button-secondary" name="edd_theme_license_deactivate" value="<?php _e('Deactivate License', 'canis'); ?>"/>
								<?php } else {
									wp_nonce_field( 'edd_sample_nonce', 'edd_sample_nonce' ); ?>
									<input type="submit" class="button-secondary" name="edd_theme_license_activate" value="<?php _e('Activate License', 'canis'); ?>"/>
								<?php } ?>
							</td>
						</tr>
					<?php } ?>
...
function canis_activate_license() {
...
add_action('admin_init', 'canis_activate_license');
...
function canis_deactivate_license() {
...
add_action('admin_init', 'canis_deactivate_license');
